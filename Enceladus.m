%% Poroviscoelastic Enceladus model, 
...
%% PHYSICAL PARAMETERS 
R0=[0 191.1e3 191.1E3+38E3 252.1E3]; %layering [m]
rhos0= [2.5E3 1000 935]; %density the solid phase kg/m^3
rhof0=[1E3 1E3 0]; %density of the fluid phase kg/m^3
mu0=[1E9 1E9 3.3E9]; %shear modulus [GPa]
Ks0=[10E9 10E9 3.3E10]; %bulk modulus solid [GPa]
Kf0=[2.2E9 2.2E9 2.2E9]; %bulkd modulus fluid incompressible [GPa]
k_perm0=[1E-4 1E-8 1E-8]; %fluid permeability 
alpha=[0.95 0 0]; %Biot's coefficient 
poro=[0.2 0 0]; %porosity 
etas0=[1e18 1e15 1e18]; % viscosity of the mantle's solid phase
etaf0=[1.9E-3 1.9E-3 1.9E-3]; % viscosity of the mantle's liquid phase
liquid=[0 1 0]; %is the layer liquid?
rhoc=2422; % average core density 
rhos0(1)=(rhoc-poro(1)*rhof0(1))/(1-poro(1)); %density of the core solid phase 
Mcore=4/3*pi*R0(2)^3*rhoc;
Mocean=4/3*pi*(R0(3)^3-R0(2)^3)*rhof0(2);
Mtotal=1.08e20;
rhos0(3)=(Mtotal-Mocean-Mcore)/(4/3*pi*(R0(end)^3-R0(end-1)^3)); %ice shell density
rho0=(1-poro).*rhos0+poro.*(rhof0); % average density 
% Orbital parameters 
omega0= 2*pi/(33*3600); %Encelauds
eccen=4.7e-3; % eccentricity Enceladus
...
[y, r]=tidal_ocean(l,R,rho,rhof,mu,Ks,etas,alpha,poro,k_perm,etaf,Kf,liquid,omega, 'radial_points',nrr,'pressure_BC',0,'resample',resample);
...
constantE=((omega0*R0(end))^2*rho0(end)/mu0(end))^2*mu0(end)/T_nd*eccen^2;
[energy_solid, energy_solid_pore,energy_fluid,energy_solid_surface, energy_solid_pore_surface,energy_fluid_surface,energy_solid_total_out, energy_fluid_total_out]=compute_energy(strain_ND,stress_ND,flux_ND,p_fluid_ND,C_fluid_ND,omega,etaf,k_perm,rr,colat,lon,poro);
Energy_Solid=mu0(end)/T_nd*constant_f^2*R0(end)^3*energy_solid_total_out;
Energy_Fluid=mu0(end)/T_nd*constant_f^2*R0(end)^3*energy_fluid_total_out;
